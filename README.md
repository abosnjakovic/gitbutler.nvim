...
A comment is tagged `stale` once the line it was written against stops matching — because the working tree moved under the review, or the changes got committed. The check runs when the pane redraws a diff, so move the status cursor off the row and back to re-evaluate a review you have left sitting. The captured line is still quoted, so the note stays locatable. Landed commits take comments too — they render through the same diff view, anchored to the commit's SHA — and since their content never changes, those notes never go stale.
...
`keys.lua` is the registry of what each key does, per context — key, action, description — that `hotbar.lua`, `hints.lua`, `buffer.lua`'s hint line, and `actions.lua`'s `?` help float all read, so a binding and what it advertises can't drift apart.
